...
class WalletTransferPage extends HookWidget {
  ...
  Widget build(BuildContext context) {
    var transferHandler = useWalletTransfer(context);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: <Widget>[
          IconButton(
            ...
            onPressed: !transferHandler.state.loading
                ? () {
      ...
      body: transferHandler.state.loading
          ? Loading()
          : TransferForm(
              address: qrcodeAddress.value,
              onSubmit: (address, amount) async {
                var success = await transferHandler.transfer(address, amount);
